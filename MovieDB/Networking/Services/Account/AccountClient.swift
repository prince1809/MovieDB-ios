...
class AccountClient: APIClient {
    ...
    func getCollectionList(page: Int, option: ProfileCollectionOption,
        ...
        let request = getCollectionListRequest(with: option, page: page, sessionId: sessionId, accountId: accountId)
    ...
    private func getCustomListRequest(with groupOption: ProfileGroupOption,
        ...
        switch groupOption {
        case .customLists:
            return AccountProvider.getCustomLists(page: page, accessToken: accessToken, accountId: accountId).request
        }
